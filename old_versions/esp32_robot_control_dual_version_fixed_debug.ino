...
// WiFi credentials
const char* ssid = "Packard";
const char* password = "I<3Rachel";
...
void setup() {
  ...
  // Setup PWM channels for ESCs using new ESP32 v3.x functions
  ledcAttach(LEFT_ESC_PIN, 50, 16);   // Pin, frequency, resolution
  ledcAttach(RIGHT_ESC_PIN, 50, 16);  // Pin, frequency, resolution
  
  // Initialize ESCs with neutral signal (1500μs pulse)
  // For 16-bit resolution at 50Hz: 1500μs = (1500/20000) * 65535 = 4915
  ledcWrite(LEFT_ESC_PIN, 4915);   // ~1500μs neutral
  ledcWrite(RIGHT_ESC_PIN, 4915);  // ~1500μs neutral
...
void setLeftMotor(float speed) {
  ...
  ledcWrite(LEFT_ESC_PIN, pwmValue);
...
void setRightMotor(float speed) {
  ...
  // Convert to 16-bit PWM value for 50Hz (20ms period)
  int pwmValue = (pulseWidth * 65535) / 20000;
  ledcWrite(RIGHT_ESC_PIN, pwmValue);
